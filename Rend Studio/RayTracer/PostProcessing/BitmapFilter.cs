...
namespace RayTracer.PostProcessing
{
    ...
    {
        public static bool Add(Bitmap b, Bitmap addition) { return Add(b, addition, 1); }

        public static bool Add(Bitmap b, Bitmap addition, float factor)
        {
            BitmapData bmData = b.LockBits(new Rectangle(0, 0, b.Width, b.Height), ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
            BitmapData bmDataAdd = addition.LockBits(new Rectangle(0, 0, addition.Width, addition.Height), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);

            int strideB = bmData.Stride;
            System.IntPtr scanB = bmData.Scan0;
            int strideAdd = bmDataAdd.Stride;
            System.IntPtr scanAdd = bmDataAdd.Scan0;

            unsafe
            {
                byte* pB = (byte*)(void*)scanB;
                byte* pAdd = (byte*)(void*)scanAdd;

                int nOffsetB = strideB - b.Width * 3;
                int nWidthB = b.Width * 3;
                int nOffsetAdd = strideAdd - addition.Width * 3;
                int nWidthAdd = addition.Width * 3;

                int nval = 0;

                for (int y = 0; y < b.Height; ++y)
                {
                    for (int x = 0; x < nWidthB; ++x)
                    {
                        nval = pB[0] + (int)(pAdd[0] * factor);
        ...
        public static bool DynamicAntiAliasing(Bitmap b) { return DynamicAntiAliasing(b, 30); }

        public static bool DynamicAntiAliasing(Bitmap b, byte nThreshold)
        {
            Bitmap edgeBm = (Bitmap)b.Clone();

            EdgeDetectDifference(edgeBm, nThreshold);

            // GDI+ still lies to us - the return format is BGR, NOT RGB.
            BitmapData bmData = b.LockBits(new Rectangle(0, 0, b.Width, b.Height), ImageLockMode.ReadWrite, PixelFormat.Format24bppRgb);
            BitmapData bmDataEdge = edgeBm.LockBits(new Rectangle(0, 0, edgeBm.Width, edgeBm.Height), ImageLockMode.ReadOnly, PixelFormat.Format24bppRgb);

            int stride = bmData.Stride;
            System.IntPtr Scan0 = bmData.Scan0;
            System.IntPtr edgeScan0 = bmDataEdge.Scan0;

            unsafe
            {
                byte* p = (byte*)(void*)Scan0;
                byte* pEdge = (byte*)(void*)edgeScan0;
                ...
                for (int y = 0; y < b.Height; ++y)
                {
                    for (int x = 0; x < b.Width; ++x)
                    {
                        if (pEdge[0] > 20 * (1 - .114 )|| pEdge[1] > 20 * (1 - .587) || pEdge[2] > 20 * (1 - .299))
                        {
                            var color = GetAntiAliasedPixel(x, y);
            ...
            b.UnlockBits(bmData);
            edgeBm.UnlockBits(bmDataEdge);

            return true;
        }

        private static AntiAliasingSamplerX4 _sampler = new AntiAliasingSamplerX4();
        private static RayTracer.Geometry.Coloring.Color GetAntiAliasedPixel(int x, int y)
        {
            return _sampler.RenderRays(new Ray[] { new Ray(RenderEngine.Scene.Camera.Position, RenderEngine.Scene.Camera.GetFrameCoordinateDirection(x, y)) })[0];

            //return RayTracer.Geometry.Coloring.Color.Red;
        }
        ...
        public static bool Smooth(Bitmap b) { return Smooth(b, 1); }
        ...
        public static bool GaussianBlur(Bitmap b) { return GaussianBlur(b, 4); }
        ...
        public static bool CloneBlur(Bitmap b) { return CloneBlur(b, 4); }

        public static bool CloneBlur(Bitmap b, int nWeight)
        {
            ConvMatrix m = new ConvMatrix();
            m.SetAll(3);
            m.Pixel = nWeight;
            m.TopMid = m.MidLeft = m.MidRight = m.BottomMid = 5;
            m.Factor = nWeight + 32;

            return BitmapFilter.Conv3x3(b, m);
        }

        public static bool MeanRemoval(Bitmap b) { return MeanRemoval(b, 9); }
        ...
        public static bool Sharpen(Bitmap b) { return Sharpen(b, 11); }
        ...
        public static bool EdgeDetectConvolution(Bitmap b, EdgeDetect nType, byte nThreshold)
        {
            ConvMatrix m = new ConvMatrix();
